    ...
    circ_template.append({
        ...
        "localizedTemplates": {
            "en": {
                ...
                "body": "<p><br></p><div><br>Dear&nbsp;{{user.firstName}} {{user.lastName}} :</div><div><br></div><div>The following is a list of current fine(s) or fee(s). More detailed information is available at the library. For payment information, please contact us at the indicated location(s). Information about fines and fees can be found at library home page.</div><div><br></div><div>{{#loans}}</div><div><strong>Location</strong>:&nbsp;{{item.effectiveLocationLibrary}}</div><div><strong>Title</strong>:&nbsp;{{item.title}}</div><div><strong>Author</strong>:&nbsp;{{item.primaryContributor}}</div><div><strong>Item ID</strong>:&nbsp;{{item.barcode}}</div><div><strong>Call #</strong>:&nbsp;{{item.callNumber}}</div><div><strong>Due Date</strong>:&nbsp;{{loan.dueDate}}</div><div><strong>Fine/Fee Date</strong>: {{feeCharge.chargeDateTime}}</div><div><strong>Description</strong>: {{feeCharge.type}}</div><div><strong>Fine/Fee Amount</strong>:&nbsp;{{feeCharge.amount}}</div><div><strong>Fine/Fee Balance</strong>:&nbsp;{{feeCharge.remainingAmount}}</div><div><br></div><div>{{/loans}}</div><div><br></div><div>If you have any questions please contact us at the indicated location.</div>",
                "attachments": []
            }
        },
